...
function SupportTableRow({
    ...
    const deleteRow = async (id) => {
        try {
            ...
            if (isConfirm) {
                await axiosBase.delete(`/supports/delete/${id}`, {
                    headers: { authorization: `Bearer ${jwtToken}` },
                });

                setSupportList((prevRoleList) => {
    ...
    const UpdateStatus = async (id, status) => {
        try {
            ...
            if (isConfirm) {
                ...
                setSupportList((prevCustomerList) => {
    ...
    return (
        <tr class="table-row">
            <td>{index + 1}</td>
            <td>{support?.userId?.company || support?.company}</td>
            <td>{support.subject}</td>
            <td>
                <select
                    value={support.status} // Set the default value to the customer's current status
                    onChange={(e) => {
                        const newStatus = e.target.value; // Get the new status from dropdown
                        UpdateStatus(support._id, newStatus); // Call UpdateStatus function with the new status
                    }}
                >
            ...
            <td>
               ...
               */}
                    <Link to={`${support._id}/chat`} className="action-button">
