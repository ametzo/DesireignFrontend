...
function CustomerTableRow({
    ...
    const deleteRow = async (id) => {
        try {
            ...
            if (isConfirm) {
                await axiosBase.delete(`/customers/delete/${id}`, {
                    headers: { authorization: `Bearer ${jwtToken}` },
                });

                setCustomerList((prevRoleList) => {
    ...
    return (
        <tr className="table-row">
            ...
            {/* <td>
                <select
                    value={customer.userStatus} // Set the default value to the customer's current status
                    onChange={(e) => {
                        const newStatus = e.target.value; // Get the new status from dropdown
                        UpdateStatus(customer._id, newStatus); // Call UpdateStatus function with the new status
                    }}
                >
