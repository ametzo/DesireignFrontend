...
function SubUserTableRow({
    ...
    const deleteRow = async (id) => {
        try {
            ...
            if (isConfirm) {
                await axiosBase.delete(`/auth/delete/${id}`, {
                    headers: { authorization: `Bearer ${jwtToken}` },
                });

                setUserList((prevRoleList) => {
    ...
    return (
        <tr class="table-row">
            ...
            <td>{user.email}</td>
            <td>
                {user?.lastLoggedIn ? formatDate(user?.lastLoggedIn) : "N/A"}
            </td>
            <td>{user.status}</td>
...
export default SubUserTableRow;
