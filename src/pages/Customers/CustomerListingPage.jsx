...
import CustomersTable from "../../features/Customer/CustomerTable";
import CustomerTable from "../../features/Leads/LeadsTable";
import usePermission from "../../hooks/usePermission";

function CustomerListingPage() {
    ...
    const fetchCustomerList = async () => {
        try {
            ...
            const response = await axiosBase.get(
                `/customers/all?skip=${filters.skip}&limit=${filters.limit}&searchQuery=${filters.searchQuery}`,
                {
            ...
            setFilters((prev) => {
                return {
                    ...prev,
                    totalCustomerList: response?.data?.totalCustomers,
                };
            });
            setIsLoading(false);
        } catch (err) {
                    ...
                    <div class="py-6">
                        ...
                        <div className="flex justify-end">
                            <Pagination
                                limit={filters?.limit}
                                skip={filters?.skip}
                                total={filters?.totalCustomerList}
                                incOrDecSkip={(number) =>
                                    setFilters((prev) => ({
                                        ...prev,
                                        skip: number,
