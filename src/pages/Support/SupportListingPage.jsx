...
import AddSupportModal from "../../features/Support/AddSupportModal";
import SupportTable from "../../features/Support/SupportTable";
import usePermission from "../../hooks/usePermission";

function SupportListingPage() {
    ...
    const fetchSupportList = async () => {
        try {
            ...
            const response = await axiosBase.get(
                `/supports/all?skip=${filters.skip}&limit=${filters.limit}&searchQuery=${filters.searchQuery}`,
                {
                    headers: { authorization: `Bearer ${jwtToken}` },
                }
            );

            setSupportList(response?.data?.supports);
            setFilters((prev) => {
                return {
                    ...prev,
                    totalSupportList: response?.data?.totalSupports,
                };
            });
            setIsLoading(false);
        } catch (err) {
                    ...
                    <div class="py-6">
                        ...
                        <div className="flex justify-end">
                            <Pagination
                                limit={filters?.limit}
                                skip={filters?.skip}
                                total={filters?.totalSupportList}
                                incOrDecSkip={(number) =>
                                    setFilters((prev) => ({
                                        ...prev,
                                        skip: number,
...
export default SupportListingPage;
